...
In the case of <<install, installing {kib} on our own>>, we need to modify Kibana config so that we can visit Kibana UI
from our another machine
...
Find the line starting with `#server.host` and https://serverfault.com/a/1050221[change that line to]
...
Then we could visit the Kibana-hosting machine anywhere from `http://server-ip:5601`
====
