...
If {kib} was in stalled via <<deb-repo,APT repository>>, the security configuration, including the password, above
**won't be initiated**.

To get the password for the user `elastic` for login, please do this in
https://discuss.elastic.co/t/where-is-the-kibana-password-set/261577/2[elasticsearch] by following referring to
https://qubitpi.github.io/elasticsearch/deb.html#deb-running-systemd[this section]
...
or if we are in a cloud environment, such as AWS, we https://stackoverflow.com/a/75955838[will need]
...
`/usr/share/elasticsearch/bin/elasticsearch-create-enrollment-token --scope kibana` as `root`
=====================
